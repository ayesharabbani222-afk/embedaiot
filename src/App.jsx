...
// Device / Product Selection (post-login gate + per-type dashboard routing)
import DeviceTypeSelection      from './pages/DeviceTypeSelection'
import OrgDeviceDashboardRouter from './pages/org/OrgDeviceDashboardRouter'
import UserDeviceDashboardRouter from './pages/user/UserDeviceDashboardRouter'
...
import OrgSources             from './pages/org/OrgSources'
import OrgAIAnalytics         from './pages/org/OrgAIAnalytics'
...
// Guards the org/user dashboard shell: requires both the role match AND a
// selected device/product type, so the Device Selection screen always shows
// first after login. Admin is unaffected — it manages every org/device type.
function DeviceGatedRoute({ children, requiredRole }) {
  const { user, activeDeviceType } = useAuth()
  if (!user) return <Navigate to="/login" replace />
  if (requiredRole && user.role !== requiredRole) return <Navigate to={`/${user.role}`} replace />
  if (!activeDeviceType) return <Navigate to={`/${requiredRole}/select-device`} replace />
  return children
}
...
function AppRoutes() {
  ...
  return (
    <Routes>
      ...
      {/* ── Device / Product Selection (shown right after org/user login) ── */}
      <Route path="/org/select-device" element={
        <ProtectedRoute requiredRole="org"><DeviceTypeSelection /></ProtectedRoute>
      } />
      <Route path="/user/select-device" element={
        <ProtectedRoute requiredRole="user"><DeviceTypeSelection /></ProtectedRoute>
      } />

      {/* ── Organization ── */}
      <Route path="/org" element={
        <DeviceGatedRoute requiredRole="org">
          <DashboardLayout navItems={orgNav} role="org" />
        </DeviceGatedRoute>
      }>
        <Route index                    element={<OrgDeviceDashboardRouter />} />
        ...
        <Route path="ai-analytics"      element={<OrgAIAnalytics />} />
        ...
        <Route path="sources"           element={<OrgSources />} />
        <Route path="custom-dashboard" element={<DashboardList />} />
        <Route path="custom-dashboard/:id" element={<DashboardEditor />} />
      </Route>
      ...
      <Route path="/user" element={
        <DeviceGatedRoute requiredRole="user">
          <DashboardLayout navItems={userNav} role="user" />
        </DeviceGatedRoute>
      }>
        <Route index                     element={<UserDeviceDashboardRouter />} />
