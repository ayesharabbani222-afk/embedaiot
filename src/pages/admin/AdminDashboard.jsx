...
import { adminStats, historicalData, devices as initialDevices, organizations, users, gateways } from '../../data/dummy'
...
export default function AdminDashboard() {
  ...
  // Currently selected organization (null = "All Devices")
  const selectedOrg = useMemo(
    () => (orgFilter === 'all' ? null : organizations.find(o => o.id === orgFilter) || null),
    [orgFilter]
  )

  // Selected org label shown on the filter button
  const activeGroupLabel = useMemo(() => {
    return selectedOrg ? selectedOrg.name : 'All Devices'
  }, [selectedOrg])

  // Filter organizations list by search query
  ...
  // Devices scoped to the selected organization (for KPI aggregation)
  const activeDevices = useMemo(() => {
    if (!selectedOrg) return deviceList
    return deviceList.filter(d => d.org === selectedOrg.name)
  }, [deviceList, selectedOrg])

  // Users / Gateways scoped to the selected organization
  const scopedUsers    = useMemo(() => (selectedOrg ? users.filter(u => u.org === selectedOrg.name) : users), [selectedOrg])
  const scopedGateways = useMemo(() => (selectedOrg ? gateways.filter(g => g.org === selectedOrg.name) : gateways), [selectedOrg])
  ...
  }, [activeDevices, deviceSearchQuery])
  ...
  // Alarms scoped to the selected organization's devices
  const scopedAlarms = useMemo(() => {
    if (!selectedOrg) return alarms
    const orgDeviceNames = new Set(activeDevices.map(d => d.name))
    return alarms.filter(a => orgDeviceNames.has(a.device))
  }, [alarms, selectedOrg, activeDevices])

  const onlineCount  = activeDevices.filter(d => d.status === 'Online' && d.switchOn).length
  ...
  return (
    <Skeleton name="admin-dashboard" loading={isLoading} transition={300}>
      <div className="space-y-6">
                  ...
                  <div className="max-h-48 overflow-y-auto divide-y divide-surface-50 dark:divide-surface-850">
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs font-bold transition-colors hover:bg-surface-50 dark:hover:bg-surface-800 ${
                        orgFilter === 'all' ? 'text-primary-600 bg-primary-50 dark:bg-primary-950/20' : 'text-surface-700 dark:text-surface-300'
                      }`}
                    >
                      All Devices
                    </button>
                    {searchedGroups.map(o => (
                      <button
                        key={o.id}
                        type="button"
                        onClick={() => {
                        ...
                        className={`w-full text-left px-3 py-2 text-xs font-bold transition-colors hover:bg-surface-50 dark:hover:bg-surface-800 flex flex-col ${
                          orgFilter === o.id ? 'text-primary-600 bg-primary-50 dark:bg-primary-950/20' : 'text-surface-700 dark:text-surface-300'
                        }`}
                      >
                        <span>{o.name}</span>
                    ...
                    {searchedGroups.length === 0 && (
                      <p className="p-3 text-xs text-center text-surface-400">No matching organizations found.</p>
                    )}
                  </div>
                </div>
              )}
        ...
        {/* 8 Stats Grid — scoped to the selected organization, or global when "All Devices" is selected */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Organizations" value={selectedOrg ? 1 : adminStats.totalOrgs}                 icon={Building2}     color="primary" trend={selectedOrg ? undefined : 2} />
          <StatCard label="Total Users"         value={selectedOrg ? scopedUsers.length : adminStats.totalUsers}       icon={Users}         color="info"    trend={selectedOrg ? undefined : 5} />
          <StatCard label="Total Devices"       value={activeDevices.length}                                    icon={Cpu}           color="neutral" />
          <StatCard label="Total Gateways"      value={selectedOrg ? scopedGateways.length : adminStats.totalGateways}  icon={Wifi}          color="neutral" />
          ...
          <StatCard label="Active Alarms"       value={scopedAlarms.length}                                     icon={AlertTriangle} color="warning" />
          <StatCard label="Total Alarms"        value={selectedOrg ? scopedAlarms.length : adminStats.totalAlarms}     icon={Activity}      color="neutral" />
        </div>
            ...
            <div className="flex items-center justify-center h-[200px] relative">
              ...
              <div className="absolute flex flex-col items-center justify-center text-center">
                <span className="text-2xl font-black text-surface-900 leading-none">
                  {Math.round((onlineCount / (activeDevices.length || 1)) * 100)}%
                </span>
