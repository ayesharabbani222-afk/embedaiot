...
const extraRows = [
  { id: 101, name: 'Low Power Factor',   org: 'FICO',            template: 'Fico Furnace',       variable: 'Power Factor',      condition: 'Less Than',    threshold: '0.85', methods: ['Email', 'WhatsApp'], message: 'Power factor is below threshold.', status: 'Active', founder: 'Zia ul Islam', updatedAt: '2026-06-20 09:14' },
  { id: 102, name: 'Phase Imbalance',    org: 'Supra Steel',     template: 'Fico Furnace',       variable: 'Voltage Phase A',   condition: 'Greater Than', threshold: '250',  methods: ['SMS'],              message: 'Phase imbalance detected.',        status: 'Active', founder: 'C Solar', updatedAt: '2026-06-22 16:40' },
  { id: 103, name: 'Overtemperature',    org: 'NUST',            template: 'EMS PANEL',          variable: 'Temperature',       condition: 'Greater Than', threshold: '75',   methods: ['Email', 'SMS'],     message: 'Temperature exceeded limit.',      status: 'Inactive', founder: 'Admin', updatedAt: '2026-06-25 11:02' },
]
...
export default function AdminTemplateTriggers() {
  ...
  const [applied, setApplied]               = useState({ org: '', trigger: '', template: '' })
  ...
  const handleSave = () => {
    ...
    if (modal === 'add') {
      setData(d => [...d, { ...form, id: Date.now(), founder: 'Admin', updatedAt: now }])
  ...
  const handleQuery = () => setApplied({ org: orgFilter, trigger: triggerQuery, template: templateQuery })

  const filtered = data.filter(r =>
    (!applied.org || r.org === applied.org) &&
    (!applied.trigger || r.name.toLowerCase().includes(applied.trigger.toLowerCase())) &&
    (!applied.template || r.template.toLowerCase().includes(applied.template.toLowerCase()))
  )
