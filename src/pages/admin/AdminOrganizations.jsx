...
import { TextInput, TextareaInput, SelectInput, CheckboxInput } from '../../components/ui/FormFields'
import HierarchyEditor from '../../components/facility/HierarchyEditor'
import { Plus, Pencil, Trash2, Eye, LogIn, Building2, Sparkles, ListTree, Boxes } from 'lucide-react'
...
export default function AdminOrganizations() {
  ...
  const [form, setForm]       = useState({ name:'', description:'', status:'Active', deviceTypes: ['ems'] })
  ...
  const openAdd  = () => {
    setForm({ name:'', description:'', status:'Active', deviceTypes: ['ems'] })
  ...
  const openEdit = (row) => {
    setSelected(row)
    setForm({ name:row.name, description:row.description, status:row.status, deviceTypes: row.deviceTypes?.length ? row.deviceTypes : ['ems'] })
  ...
  const columns = [
    ...
    {
      key:'deviceTypes', label:'Device/Product Types',
      render: (v) => (
        <div className="flex flex-wrap gap-1">
          {(v?.length ? v : ['ems']).map(id => {
            const dt = DEVICE_TYPES.find(d => d.id === id)
            return dt ? <span key={id} className="badge badge-info">{dt.shortName}</span> : null
          })}
        </div>
      ),
    },
        ...
        <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
          ...
          <div className="pt-4 border-t border-surface-100">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <label className="text-xs font-bold text-surface-800 uppercase tracking-wide">Device / Product Types</label>
            </div>
            <p className="text-xs text-surface-500 mb-3">
              Which Embed AIoT device/product types does this organization have? Selected types will appear on their Device Selection screen after login.
            </p>
              ...
              {DEVICE_TYPES.map(dt => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {dt.provisional && <p className="text-[10px] text-surface-400">Parameters pending</p>}
                  </div>
                </label>
              ))}
            </div>
        ...
        {selected && (
          <div className="space-y-3">
            ...
            <div className="flex gap-4">
              <span className="text-xs text-surface-500 w-28 flex-shrink-0">Device Types</span>
              <div className="flex flex-wrap gap-1">
                {(selected.deviceTypes?.length ? selected.deviceTypes : ['ems']).map(id => {
                  const dt = DEVICE_TYPES.find(d => d.id === id)
                  return dt ? <span key={id} className="badge badge-info">{dt.shortName}</span> : null
                })}
              </div>
