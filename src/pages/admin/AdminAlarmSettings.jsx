...
const initialData = initialRaw.map(r => ({
  ...r,
  pushBody: `Alert triggered: ${r.name}`,
  founder: 'Admin',
}))

const blankForm = {
  name: '', org: '', pushType: 'Template Trigger', pushBody: '', pushMethod: 'Email',
...
export default function AdminAlarmSettings() {
  ...
  const [triggerFilter, setTriggerFilter] = useState('Template Trigger')
  const [nameQuery, setNameQuery]         = useState('')
  const [applied, setApplied]             = useState({ org: '', trigger: '', name: '' })
  ...
  const handleBatchDelete = () => {
    ...
    if (confirm(`Delete ${selectedIds.length} selected setting(s)?`)) {
      setData(d => d.filter(r => !selectedIds.includes(r.id)))
      setSelectedIds([])
    }
  }

  const handleQuery = () => setApplied({ org: orgFilter, trigger: triggerFilter, name: nameQuery })

  const filtered = data.filter(r =>
    (!applied.org || r.org === applied.org) &&
    (!applied.trigger || r.pushType === applied.trigger) &&
    ...
    <div>
      ...
      <div className="card p-4 mb-5">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="w-44">
            <SelectInput label="Push Type" value={triggerFilter}
