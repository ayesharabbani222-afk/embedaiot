...
import { devices as initialData, organizations, gateways, deviceTemplates, users } from '../../data/dummy'
...
export default function AdminDevices() {
  ...
  const handleBatchDelete = () => {
    ...
    if (confirm(`Delete ${selectedIds.length} selected device(s)?`)) {
  ...
  const handleExport = () => {
    ...
    const rows = filtered.map(r => [r.name, r.org, r.gateway, r.template, r.status, r.switchOn ? 'On' : 'Off'])
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="w-40">
            <SelectInput label="Status" placeholder="All Status" value={statusFilter}
              ...
              options={['Online', 'Offline', 'Alarm', 'Not Configured']} />
          </div>
