...
import { gateways as initialGateways } from '../../data/dummy'
import { useAuth } from '../../context/AuthContext'

function loadGateways() {
  try {
    const saved = localStorage.getItem('cf-ems-gateways')
    return saved ? JSON.parse(saved) : initialGateways
  } catch {
...
export default function OrgGateways() {
  ...
  const [allGateways, setAllGateways] = useState(loadGateways)

  useEffect(() => {
    localStorage.setItem('cf-ems-gateways', JSON.stringify(allGateways))
  }, [allGateways])

  const data = useMemo(() => allGateways.filter(g => g.org === orgName), [allGateways, orgName])
  ...
  const handleSave = () => {
    if (modal === 'add') {
      setAllGateways(d => [...d, { id: Date.now(), devices: 0, org: orgName, ...form }])
    } else {
      setAllGateways(d => d.map(r => r.id === selected.id ? { ...r, ...form } : r))
    }
    close()
  }
