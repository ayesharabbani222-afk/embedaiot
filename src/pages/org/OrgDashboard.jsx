...
export default function OrgDashboard() {
  ...
  // Sources (from the Sources page / SourceContext) — feed the Power Flow mind map's
  // Level 1 (Grid / Solar / Generator / Battery / custom). Only Active sources are shown.
  const { getActiveSourcesForOrg, updateSource, deleteSource } = useSources()
  const powerFlowSources = useMemo(() => getActiveSourcesForOrg(orgName), [getActiveSourcesForOrg, orgName])

  const handleUpdateSource = (id, patch) => updateSource(id, patch)
  const handleDeleteSource = (id) => deleteSource(id)
